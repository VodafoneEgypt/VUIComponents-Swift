...
+(NSString *)GetDateWithCustomFormat:(double)timestamp{
    ...
    NSLocale *usLocale = [[NSLocale alloc] initWithLocaleIdentifier:([LanguageHandler sharedInstance].currentLanguage == ARABIC)?@"ar":@"en_US"];
...
+ (NSString *) getCurrentLanguage {
    return ([[LanguageHandler sharedInstance] currentLanguage] == ENGLISH) ? @"en" : @"ar";
}
...
+(UIFont*)getFontWithName:(NSString*)fontName AndSize:(NSInteger)size{
    ...
    return [UIFont fontWithName:fontName size: size - (([LanguageHandler sharedInstance].currentLanguage == ARABIC) ? 2 : 0)];
}
...
+(NSString *)getIntervalStringSinceDate:(NSDate *)date{
    ...
    NSDateComponents *components = [calendar components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit| NSHourCalendarUnit | NSMinuteCalendarUnit | NSSecondCalendarUnit fromDate:date toDate:currentDate options:0];
