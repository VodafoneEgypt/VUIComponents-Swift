...
@implementation KAProgressLabelVF {
                       ...
                       context:(void *)context
{
    ...
    if([keyPath isEqualToString:@"startDegree"] ||
       [keyPath isEqualToString:@"endDegree"]){
        ...
        KAProgressLabelVF *__unsafe_unretained weakSelf = self;
...
-(void)drawProgressLabelCircleInRect:(CGRect)rect
{
    ...
    if(isnan(_endDegree) || isinf(_endDegree)){
        self.endDegree = 0;
    }
    
    if(isnan(_startDegree) || isinf(_startDegree)){
        self.startDegree = 0;
    }
    ...
    self.endLabel.frame =  [self rectForDegree:_endDegree-1 andRect:rect];
    
    self.startLabel.layer.cornerRadius = [self borderDelta];
    self.endLabel.layer.cornerRadius = [self borderDelta];

    CGRect tempRect = self.endLabel.frame ;
    
    CGFloat triangleAngle = progressEndAngle + KADegreesToRadians(90);
    
    self.endLabel.transform = CGAffineTransformMakeRotation(0);
        
    [self.endLabel viewWithTag:910].transform = CGAffineTransformMakeRotation(triangleAngle);
    
    
    //self.endLabel.frame = CGRectMake(self.endLabel.frame.origin.x, self.endLabel.frame.origin.y,6, 6) ;
    
    //NSLog(@"%f , %f" ,self.endLabel.bounds.size.height , self.endLabel.bounds.size.width);
...
- (CGRect) rectForDegree:(float) degree andRect:(CGRect) rect
{
    ...
    return CGRectMake(x + KADegreesToRadians(_roundedCornersWidth), y + KADegreesToRadians(_roundedCornersWidth), _roundedCornersWidth, _roundedCornersWidth);
}
