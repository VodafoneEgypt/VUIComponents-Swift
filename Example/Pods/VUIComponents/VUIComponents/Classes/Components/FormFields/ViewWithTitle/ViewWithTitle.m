...
-(void)setTitle:(NSString *)title{
    ...
    _titleLabel.textAlignment =( [LanguageHandler sharedInstance].currentDirection == RTL )?NSTextAlignmentRight:NSTextAlignmentLeft;
...
- (void) setContentView:(UIView*)view{
    ...
    if ([view isKindOfClass:[ValidationTextField class]]) {
        ...
        ((ValidationTextField*)view).heightDidChangedBlock = ^(CGFloat height){
            ...
            CGRect cutomViewFrame = cutomView.frame;
            
            expandedHeight = height;
            if (((ValidationTextField*)view).toolTibPosition == UP) {
                
                cutomViewFrame.origin.y = expandedHeight;
                cutomView.frame = cutomViewFrame;
            }
