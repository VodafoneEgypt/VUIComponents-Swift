...
-(void)tapDetected {
    ...
    CGFloat dropDownHeigh =  ((MIN(_data.count, 3)) * self.bounds.size.height);
    
    CGRect frame = self.frame;
    frame.size.height = dropDownHeigh;
    
    dropDownViewController = [[DropDownViewController alloc] initDropDownWithFrame:frame];
    
    dropDownViewController.preferredContentSize = CGSizeMake(self.bounds.size.width, dropDownHeigh);
